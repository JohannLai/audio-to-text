...
if [[ -z "$API_KEY" ]]; then
  ...
  if [[ -f ".env" ]]; then
    ...
    # double check if API_KEY environment variable is set
    if [[ -z "$API_KEY" ]]; then
      echo "API_KEY environment variable is not set in .env file"
      exit 1
    fi
  else
    echo "API_KEY environment variable is not set and .env file does not exist"
    exit 1
  fi
fi
